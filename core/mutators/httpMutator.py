...
class HttpMutator(Mutator):
    ...
    def __init__(self, nodes_to_mutate: Dict[str, str], root: ABNFToken, non_terminals: List[ABNFToken], min_count: int = 0, max_count: int = 0, random_seed: int = 123, verbose: bool = False) -> None:

        Mutator.__init__(root, non_terminals, min_count,
                         max_count, random_seed)
    ...
    def mutate(self) -> None:
        ...
        if self.nodes_to_mutate[zoombie_node.value] not in mutationTypes.types:
            raise(f"Unrecognizable mutation type {self.nodes_to_mutate[zoombie_node.value]} for node {zoombie_node.value}")

        if self.nodes_to_mutate[zoombie_node.value] == mutationTypes.types[0]:
            # point mutation
            point_mutation_variant = random.choice(mutationTypes.pointMutations)
            
            # randomly select a expansion value for the zoombie node
            zoombie_node.children[0] = random.choice(zoombie_node.children)
            ...
            # genetic mutation
            genetic_mutation_variant = random.choice(mutationTypes.geneticMutations)
