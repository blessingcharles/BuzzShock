...
class Logger:
    ...
    def log(self, msg: str, symbol: str = "+"):
        ...
        if self.is_color:
            msg = colorama.Style.BRIGHT + colorama.Fore.MAGENTA + \
                "[%s] %s" % (colorama.Fore.CYAN+symbol +
                             colorama.Fore.MAGENTA, msg) + colorama.Style.RESET_ALL
            print(msg)
        else:
            print(plaintext)
...
class Bzlogger:
    ...
    def info(message: str, *args):
        Bzlogger.print_general(color=colorama.Fore.BLUE,
                               tag="[*] ", message=message, *args)

    @staticmethod
    def error(message: str, *args):
        Bzlogger.print_general(color=colorama.Fore.RED,
                               tag="[-] ", message=message, *args)

    @staticmethod
    def failed(message: str, *args):
        Bzlogger.print_general(
            color=colorama.Fore.LIGHTYELLOW_EX, tag="(-) ", message=message, *args)

    @staticmethod
    def success(message: str, *args):
        Bzlogger.print_general(color=colorama.Fore.GREEN,
                               tag="[+] ", message=message, *args)

    @staticmethod
    def warning(message: str, *args):
        Bzlogger.print_general(color=colorama.Fore.YELLOW,
                               tag="[#] ", message=message, *args)

    @staticmethod
    def checker(message: str, *args):
        Bzlogger.print_general(color=colorama.Fore.LIGHTMAGENTA_EX,
                               tag="[.] ", message=message, *args)

    @staticmethod
    def printer(message: str, *args):
        Bzlogger.print_general(
            color=colorama.Fore.LIGHTBLUE_EX, tag="", message=message, *args)
